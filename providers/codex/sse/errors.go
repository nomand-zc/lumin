...
func ParseErrorResponse(statusCode int, body []byte) error {
	...
	if err := json.Unmarshal(body, &errWrapper); err == nil && errWrapper.Error != nil {
		msg := fmt.Sprintf("Codex API error [%s]: %s", errWrapper.Error.Code, errWrapper.Error.Message)
		return newHTTPError(statusCode, msg, body)
	}

	return newHTTPError(statusCode,
		fmt.Sprintf("Codex API error, status=%d", statusCode), body)
}
